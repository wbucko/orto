...
SSH_USER="${SSH_USER:-wojtek}"
KEY_USER="${KEY_USER:-wojtek}"
DOCKER_VERSION="${DOCKER_VERSION:-17.05.0}"

DOCKER_PULL_IMAGES=("postgres:9.6" "redis:3.2")
...
wget -O "docker.deb https://apt.dockerproject.org/repo/pool/main/d/docker-engine/docker-engine_${1}~ce-0~debian-stretch_amd64.deb"
...
function git_init () {
  ...
  scp "git/post-receive/orto" "${SSH_USER}@${SERVER_IP}:/tmp/orto"
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
sudo apt-get update && sudo apt-get install -y -q git
sudo rm -rf /var/git/orto.git /var/git/orto
sudo mkdir -p /var/git/orto.git /var/git/orto
sudo git --git-dir=/var/git/orto.git --bare init

sudo mv /tmp/orto /var/git/orto.git/hooks/post-receive
sudo chmod +x /var/git/orto.git/hooks/post-receive
sudo chown ${SSH_USER}:${SSH_USER} -R /var/git/orto.git /var/git/orto
  '"
  echo "done!"
}
